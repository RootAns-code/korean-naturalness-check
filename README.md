...
| 4. 어색 어휘 사전 | 정규식 사전(`AWKWARD_PATTERNS`) 매칭 |
| 5. 코퍼스 결합 빈도 | 동봉 DB에서 (명사, 동사) 결합 빈도 룩업 — 사전에 없는 어색 결합도 자동 검출 |

시그널 5의 참조 데이터(`assets/kor_collocation.db`, 약 3.9MB)는 Leipzig kor_news_2022_1M 코퍼스를 kiwipiepy로 형태소 분석해 빌드한 명사+동사 결합 빈도 SQLite DB입니다(52,801 페어). 빌드 절차는 `scripts/build_collocation_data.py` 참고 — 사용자는 실행할 필요 없습니다.
...
- 같은 유형 페어 검증 100%, OOD 페어 71%, 자연 문장 거짓 양성 0%
- 거짓 양성 0%가 핵심: 의심으로 보고된 문장은 실제로 어색할 확률이 높습니다
