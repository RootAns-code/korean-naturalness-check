        ...
        'review_required': [
            {
                ...
                'test': (
                    ...
                    '(b) lead에만 있던 사실이 사라지면 삭제하지 않는다. 단, 구조 검사는 계속한다. '
                    '(c) 사라지는 것이 주어뿐이면 그 주어를 follow 안에 넣어 합칠 수 있다 → 대상. '
                    '이어서 두 문장이 원인과 결과, 조건과 결론, 배경과 판단, 같은 대상의 설명 관계인지 확인한다. '
                    '관계가 있으면 두 문장의 정보를 보존한 재구성문을 실제로 쓴다. '
                    '원인이나 조건을 앞에 놓고 결과를 뒤에 두거나 같은 대상의 설명을 통합한다. '
                    '원문 유지에는 재구성문과 비교한 구체적 이유가 필요하며 독립 정보라는 이유만으로 통과시키지 않는다. '
                    '관계가 없는 병렬 사실은 억지로 합치지 않는다. '
                    'first_person_lead는 화자 보존 참고값이며 구조 검사 면제 사유가 아니다. '
                    '경험과 의견의 주체는 재구성문에도 보존한다.'),
                'prescription': (
                    '대상으로 판정한 쌍만 고친다. '
                    ...
                    '인과관계를 새로 만들거나 사실, 조건, 확실성을 바꾸지 않는다. '
                    '합치면 과도하게 길어지거나 문맥상 결론을 먼저 강조할 필요가 있으면 비교 근거를 남기고 유지한다. '
                    '전부 없애지는 마라. 선언 전멸은 과교정이며, '
                    '문단이 무엇을 다루는지 알려주는 기능까지 사라진다.'),
                'reporting': (
                    '판정 결과를 쌍 단위로 남겨라. 대상이 아니라고 본 쌍도 위 test의 '
                    '삭제 결과와 문장 관계를 적고, 관계가 있으면 재구성문과 채택 또는 유지 이유를 남긴다. 대상만 골라 보고하면 '
                    '순회를 건너뛴 것과 구분되지 않는다.'),
            },
            {
                'id': 'cleft_structure',
...
def print_text_report(result: dict, top: int = 10):
    ...
    if dp.get('active'):
        ...
        if pairs:
            ...
            for p in pairs:
                fp = ' [1인칭 보존]' if p['first_person_lead'] else ''
